  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background: "#1e293b", borderRadius: 16, width: 560, maxWidth: "95vw",
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #334155", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 800 }}>📊 End of Day Report</div>
            {report && <div style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>{report.date} · Generated at {report.generated_at}</div>}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => window.print()} className="receipt-btn print" style={{ padding: "8px 16px" }}>🖨️ Print</button>
            <button onClick={onClose} className="receipt-btn close" style={{ padding: "8px 16px" }}>✕</button>
          </div>
        </div>

        {/* Body */}
        <div style={{ padding: "20px 24px" }}>
          {loading && <p style={{ color: "#94a3b8", textAlign: "center", padding: "40px 0" }}>⏳ Loading report...</p>}
          {error  && <p style={{ color: "#ef4444", textAlign: "center", padding: "40px 0" }}>❌ {error}</p>}
          ...
          {report && (
            <>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
                ...
                {report.total_profit > 0 && card("Est. Profit", fmt(report.total_profit), "#a78bfa", `Cost: ${fmt(report.total_cost)}`)}
              </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  {report.top_by_revenue.map((p, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: "1px solid #0f172a" }}>
                      <span style={{ width: 24, height: 24, background: "#0f172a", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#64748b", flexShrink: 0 }}>{i + 1}</span>
                      <span style={{ flex: 1, fontSize: 14, fontWeight: 500 }}>{p.name}</span>
                      <span style={{ fontWeight: 800, color: "#10b981" }}>{fmt(p.revenue)}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div>
                  ...
                  {report.top_by_qty.map((p, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: "1px solid #0f172a" }}>
                      <span style={{ width: 24, height: 24, background: "#0f172a", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, color: "#64748b", flexShrink: 0 }}>{i + 1}</span>
                      <span style={{ flex: 1, fontSize: 14, fontWeight: 500 }}>{p.name}</span>
                      <span style={{ fontWeight: 800, color: "#3b82f6" }}>{p.qty} units</span>
                    </div>
                  ))}
                </div>
              )}
