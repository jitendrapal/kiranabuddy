...
export function addRegularItem(cart, code, name, price, mode) {
  ...
  return [{ code, name, price: parseFloat(price) || 0, delta, isWeight: false }, ...cart];
}

/** Add a weight-based item to cart */
export function addWeightItem(cart, { code, name, price, pricePerKg, weightKg, emoji, mode }) {
  const delta = mode === 'add' ? 1 : -1;
  const cartKey = `${code}_${Date.now()}`;
  const newItem = {
    code: cartKey, displayCode: code, name, price: parseFloat((pricePerKg * weightKg).toFixed(2)),
    delta, isWeight: true, weightKg, pricePerKg, emoji,
  };
  return [newItem, ...cart];
}
